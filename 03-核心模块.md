...
> **ID 策略说明**：本系统统一使用 **nanoid(12)** 作为主键：
>
> - **长度固定**: 12 字符（如 `k2x9pL4mN5qR`）
> - **数据库**: `CHAR(12)` 类型，索引性能好
> - **API/缓存**: 直接使用 nanoid，无需转换
> - **安全性**: 64^12 ≈ 4.7千万亿亿种组合，18年内碰撞概率 < 1%
> - **用户友好**: URL 短小，易分享
>
> 详见 [§ 3.8 ID 生成工具](#38-id-生成工具)
...
fn extract_user_from_request<T>(request: &Request<T>) -> Result<String, Status> {
    let token = extract_jwt_from_metadata(request.metadata())?;
...
impl MemberService {
    ...
    pub async fn update_permissions(
        &self,
        room_id: &str,          // nanoid(12)
        target_user_id: &str,   // nanoid(12)
        operator_id: &str,      // nanoid(12)
        permissions: u64,
...
impl ClusterSync {
    ...
    pub async fn publish(&self, event: ClusterEvent) -> Result<()> {
        let envelope = ClusterEventEnvelope {
            event_id: generate_id(),  // nanoid(12)
...
- ✅ **全系统使用 nanoid(12)** - 数据库、API、缓存统一
- ✅ **固定长度 12 字符** - 如 `k2x9pL4mN5qR`
- ✅ **数据库 CHAR(12)** - 固定长度，索引性能好
- ✅ **URL 友好** - 无需转义，易分享
- ✅ **安全性充足** - 64^12 组合，18年内碰撞概率 < 1%

**优势**：

- ✅ 架构简单 - 无需 ID 映射和转换
- ✅ 性能更好 - 减少一层查询和转换开销
- ✅ 代码更清晰 - 数据库 ID 直接用于 API
- ✅ 用户友好 - 短小精悍，易于记忆
...
mod tests {
    ...
    #[test]
    fn test_validate_id() {
        assert!(is_valid_id("k2x9pL4mN5qR"));
        assert!(is_valid_id("A1B2C3D4E5F6"));

        // 无效的 ID
        assert!(!is_valid_id("too-short"));  // 太短
        assert!(!is_valid_id("way-too-long-id"));  // 太长
        assert!(!is_valid_id("invalid@chars"));  // 包含非法字符
    }
...
**所有数据模型直接使用 String 类型的 nanoid**：

```rust
// model/user.rs
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct User {
    /// nanoid(12) - 直接用于数据库和 API
    pub id: String,
...
impl RoomService {
    ...
    /// 批量查询 - 无需映射转换
    pub async fn get_rooms(&self, room_ids: &[String]) -> Result<Vec<Room>> {
        self.room_repo.get_many(room_ids).await
    }
}
```
