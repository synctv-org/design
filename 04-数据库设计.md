...
CREATE TABLE rooms (
    ...
    creator_id CHAR(12) NOT NULL REFERENCES users(id) ON DELETE CASCADE,
...
CREATE TABLE playlists (
    ...
    -- 排序位置（支持手动调整目录顺序）
    -- 添加时自动计算：如果未指定，则为父目录中最大 playlist position + 1
    -- 列表显示：先显示所有 playlist（按 position 排序），再显示所有 media
...
impl UserProviderCredential {
    ...
    pub async fn get_bilibili(user_id: String) -> Result<Option<Self>> {
    ...
    pub async fn get_alist(user_id: String, server_id: &str) -> Result<Option<Self>> {
        let cred = user_provider_credentials::table
            .filter(user_provider_credentials::user_id.eq(user_id))
    ...
    pub async fn list_alist_servers(user_id: String) -> Result<Vec<Self>> {
...
CREATE TABLE medias (
    ...
    -- 排序位置（支持手动调整播放顺序）
    -- 添加时自动计算：如果未指定，则为当前 playlist 中最大 media position + 1
    -- 列表显示：在所有 playlist 之后显示（按 position 排序）
    position INT NOT NULL DEFAULT 0,
...
CREATE TABLE room_playback_session (
    ...
    playlist_id CHAR(12) REFERENCES playlists(id) ON DELETE SET NULL,

    -- 静态媒体：引用具体的 media 记录
    media_id CHAR(12) REFERENCES medias(id) ON DELETE SET NULL,
    ...
    -- 最后更新时间（用于时间同步）
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_by CHAR(12) REFERENCES users(id)
);
...
CREATE TABLE streams (
    ...
    media_id CHAR(12) NOT NULL REFERENCES medias(id) ON DELETE CASCADE,
...
1. **用户表**: nanoid(12)主键、OAuth2支持、邮箱验证、审计字段
...
**设计原则**: 类似文件管理器，文件夹和文件分开管理 position，列表显示时文件夹在前。

#### Position 自动计算

**添加 Playlist 时**:
```sql
-- 如果用户未指定 position，自动计算
-- 查找父目录中所有 playlist 的最大 position + 1（包括静态和动态）
...
```sql
-- 如果用户未指定 position，自动计算
-- 查找当前 playlist 中所有 media 的最大 position + 1
SELECT COALESCE(MAX(position), -1) + 1
FROM medias
WHERE playlist_id = $1;
```
...
**查询规则**: 先显示所有 playlist，再显示所有 media

```sql
-- 列出某个目录的内容
SELECT * FROM (
    -- 1. 子 playlist（文件夹，包括静态和动态）
    SELECT
        'playlist' as type,
        id, name, position,
...
```
权力的游戏/
├── [文件夹] 第一季/          position=0, type_order=0
├── [文件夹] 第二季/          position=1, type_order=0
├── [文件夹] 第三季/          position=2, type_order=0
├── [视频] 特别篇.mp4        position=0, type_order=1
└── [视频] 幕后花絮.mp4      position=1, type_order=1
```

**API 过滤支持**:
- `?type=playlist`: 只显示子文件夹
- `?type=media`: 只显示媒体文件
- `?type=all` 或不传: 显示全部（文件夹在前）

#### Position 独立性

- ✅ Playlist 和 Media 的 position 互不影响
- ✅ Playlist position 在同一父目录下唯一递增
- ✅ Media position 在同一 playlist 下唯一递增
- ✅ 允许手动调整 position（拖拽排序）
- ✅ 删除项目不影响其他项目的 position
...
- ✅ **Position 独立管理**: Playlist 和 Media 各自维护排序位置
- ✅ **文件夹优先显示**: 查询时自动将文件夹排在文件前面
