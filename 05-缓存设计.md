...
impl KeyBuilder {
    ...
    /// nanoid -> UUID 映射
    pub fn nanoid_to_uuid(&self, nanoid: &str) -> String {
        format!("{}:nanoid:{}", self.prefix, nanoid)
    }

    /// UUID -> nanoid 映射
    pub fn uuid_to_nanoid(&self, uuid: &Uuid) -> String {
        format!("{}:uuid:{}", self.prefix, uuid)
    }
...
### 5.0.3 ID 映射系统（UUID ↔ nanoid）

**设计目标**：

- 数据库使用 UUID（PostgreSQL 原生支持，性能好）
- API/缓存使用 nanoid（短小，用户友好）
- 双向映射，自动缓存
...
impl IdMapper {
    pub fn new(redis: redis::Client, key_builder: KeyBuilder) -> Self {
        Self { redis, key_builder }
    }

    /// UUID -> nanoid（有缓存则返回，无则生成并缓存）
    ///
    /// # Example
    /// ```rust
    /// let room_uuid = Uuid::new_v4();
    /// let room_nanoid = id_mapper.uuid_to_nanoid(&room_uuid).await?;
    /// // 返回: "k2x9pL4mN5qR" (12字符)
    /// ```
    pub async fn uuid_to_nanoid(&self, uuid: &Uuid) -> Result<String> {
        let mut conn = self.redis.get_async_connection().await?;
        let key = self.key_builder.uuid_to_nanoid(uuid);

        // 尝试从缓存获取
        if let Some(nanoid) = conn.get::<_, Option<String>>(&key).await? {
            return Ok(nanoid);
        }

        // 生成新的 nanoid（12字符，安全性足够）
        let nanoid = nanoid!(12);

        // 双向缓存（永久存储）
        conn.set(&key, &nanoid).await?;
        conn.set(&self.key_builder.nanoid_to_uuid(&nanoid), &uuid.to_string()).await?;
    ...
    /// # Example
    /// ```rust
    /// let room_uuid = id_mapper.nanoid_to_uuid("k2x9pL4mN5qR").await?;
    /// ```
    pub async fn nanoid_to_uuid(&self, nanoid: &str) -> Result<Uuid> {
        let mut conn = self.redis.get_async_connection().await?;
        let key = self.key_builder.nanoid_to_uuid(nanoid);

        let uuid_str = conn.get::<_, Option<String>>(&key).await?
            .ok_or_else(|| Error::InvalidNanoid(nanoid.to_string()))?;
    ...
    pub async fn batch_uuid_to_nanoid(&self, uuids: &[Uuid]) -> Result<Vec<String>> {
        ...
        for uuid in uuids {
            results.push(self.uuid_to_nanoid(uuid).await?);
        }
        Ok(results)
    }

    /// 批量 nanoid -> UUID
    pub async fn batch_nanoid_to_uuid(&self, nanoids: &[&str]) -> Result<Vec<Uuid>> {
        let mut results = Vec::with_capacity(nanoids.len());
        for nanoid in nanoids {
            results.push(self.nanoid_to_uuid(nanoid).await?);
        }
        Ok(results)
    }
}
...
async fn get_room_handler(
    ...
    Path(room_nanoid): Path<String>,
) -> Result<Json<RoomResponse>> {
    // 1. nanoid -> UUID
    let room_uuid = state.id_mapper.nanoid_to_uuid(&room_nanoid).await?;

    // 2. 从数据库查询（使用 UUID）
    let room = state.db.get_room_by_uuid(&room_uuid).await?;

    // 3. 转换为 API 响应（使用 nanoid）
    let response = RoomResponse {
        id: room_nanoid,  // 返回 nanoid 给客户端
        name: room.name,
        creator_id: state.id_mapper.uuid_to_nanoid(&room.creator_id).await?,
        // ...
    };
...
async fn create_room_handler(
    ...
    // 2. 生成 nanoid（自动缓存映射）
    let room_nanoid = state.id_mapper.uuid_to_nanoid(&room_uuid).await?;
...
|------|---------|------|-----|
| **ID 映射** |
| nanoid→UUID | `{prefix}:nanoid:{nanoid}` | String | 永久 |
| UUID→nanoid | `{prefix}:uuid:{uuid}` | String | 永久 |
| **集群** |
...
async fn update_viewer_count(room_id: &str, count: usize) -> Result<()> {
    let key = format!("room:viewers:{}", room_id);  // room_id 是 nanoid string
...
pub async fn get_room(&self, room_id: &str) -> Result<Room> {
    // 1. 尝试从缓存获取 (room_id 是 nanoid string)
...
pub struct DanmakuCache {
    cache: Arc<DashMap<i64, RwLock<VecDeque<Danmaku>>>>,  // room_id -> 弹幕队列
...
impl DanmakuCache {
    ...
    /// 添加弹幕到缓存
    pub async fn add(&self, room_id: &str, danmaku: Danmaku) {  // room_id 是 nanoid string
    ...
    /// 获取最近的弹幕（用于新用户加入）
    pub async fn get_recent(&self, room_id: &str) -> Vec<Danmaku> {  // room_id 是 nanoid string
    ...
    /// 清空房间缓存
    pub fn clear(&self, room_id: &str) {  // room_id 是 nanoid string
