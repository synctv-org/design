...
async fn list_playlist_items(
    State(state): State<AppState>,
    Path((room_id, playlist_id)): Path<(Uuid, Uuid)>,
...
impl PlaybackService {
    ...
    pub async fn play(
        ...
        // 2. 验证 playlist 属于该房间
        let playlist_uuid = self.id_mapper.nanoid_to_uuid(&playlist_id).await?;
        let playlist = self.playlist_repo.get(playlist_uuid).await?;

        let room_uuid = self.id_mapper.nanoid_to_uuid(room_id).await?;
        if playlist.room_id != room_uuid {
        ...
        match source {
            PlaySource::StaticMedia { media_id } => {
                // 转换 nanoid -> UUID
                let media_uuid = self.id_mapper.nanoid_to_uuid(&media_id).await?;
...
更新 room_playback_session
    playlist_id = playlist_xyz (UUID)
    media_id = media_123 (UUID)
    relative_path = NULL
...
POST /api/v1/rooms/abc123/play
{
  ...
  "relative_path": "/action/movie.mp4"
    ...
    ↓
更新 room_playback_session
    playlist_id = alist_playlist (UUID)
    media_id = NULL
    relative_path = "/action/movie.mp4"
    ↓
获取播放信息时合并配置
...
pub enum SubtitleSource {
    ...
    Embedded,                       // 视频内嵌
    Generated,                      // 自动生成（AI）
    UserUploaded { user_id: Uuid },  // 用户上传
}
...
impl CredentialStore {
    /// 获取用户在某平台的凭证
    pub async fn get(&self, source_type: &str, user_id: Uuid) -> Result<Credential> {
        // 查询数据库
