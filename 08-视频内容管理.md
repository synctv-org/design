...
pub trait DynamicFolder: MediaProvider {
    ...
    ///
    /// # 使用场景
    /// 当播放的是动态文件夹（如 Emby 播放列表、Alist 文件夹）时，
    /// 自动连播系统调用此方法获取下一个要播放的项目。
    ///
    /// # 参数
    /// - `ctx`: Provider 上下文
    /// - `playlist`: 动态文件夹对象
    /// - `current_media`: 当前播放的媒体对象
    /// - `mode`: 播放模式（顺序、随机、循环等）
    ///
    /// # 返回
    /// - `Some(NextPlayItem)`: 下一个播放项的信息
    /// - `None`: 没有更多内容（顺序播放结束）
    ///
    /// # 实现要点
    /// 1. **状态管理**: Provider 需要维护播放位置状态（可存储在 Media.source_config 中）
    /// 2. **分页加载**: 动态加载内容，不预先加载所有项目
    /// 3. **播放模式**: 根据 mode 参数返回合适的下一项
    /// 4. **缓存优化**: 缓存已加载的目录内容，避免重复请求
    ///
    /// # 示例
    /// ```rust
    /// // Emby 播放列表场景
    /// // current_media.source_config = {"playlist_id": "123", "current_index": 5}
    /// // 调用 next() 返回 index=6 的项目
    ///
    /// // Alist 文件夹场景
    /// // current_media.source_config = {"folder_path": "/movies", "current_file": "movie_05.mp4"}
    /// // 调用 next() 返回文件夹中的下一个视频文件
    /// ```
    async fn next(
        &self,
        ctx: &ProviderContext<'_>,
...
pub struct NextPlayItem {
    ...
    /// Provider source_config（用于生成播放信息）
    /// 存储到 Media 表的 source_config 字段
    pub source_config: Value,

    /// 元数据（缩略图、时长等）
    pub metadata: Metadata,

    /// Provider 内部数据（用于下次调用 next）
    /// 例如：Emby 播放列表的 index，Alist 文件夹的 current_path
    #[serde(skip_serializing)]
    pub provider_data: Value,
}
