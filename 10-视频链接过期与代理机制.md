...
// 从动态配置读取（vendor-specific 配置使用 get_dynamic，返回 Result）
impl VideoConfig {
    pub async fn from_settings(
        ...
        Ok(Self {
            proxy_enabled: settings
                .get_dynamic(&format!("proxy.enable_{}", vendor), "false")?,
            cache_size: settings
                .get_dynamic(&format!("proxy.cache_size_{}", vendor), "1073741824")? as u64,  // 1GB
            enable_cache: settings
                .get_dynamic(&format!("proxy.enable_cache_{}", vendor), "true")?,
        })
    }
}
```
